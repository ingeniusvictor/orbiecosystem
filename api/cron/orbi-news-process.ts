...
export default async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    const result = await executeVercelLiveProcessingCron({ environment: process.env });
    json(res, 200, result);
  } catch (error) {
    json(res, 503, { error: error instanceof Error ? error.message : 'VERCEL_LIVE_PROCESSING_CRON_FAILED' });
  }
}
