...
import { createLiveArticleFetcher } from '../news/live-article-fetcher';
...
export const runVercelLiveProcessingPreflight = (environment: VercelLiveProcessingEnvironment) => {
  ...
  if (profile !== ControlledActivationProfile.EDITORIAL_ASSISTED) reasons.push('VERCEL_LIVE_PROCESSING_PROFILE_MUST_BE_EDITORIAL_ASSISTED');
  ...
  for (const action of [OperationalAction.VERIFY_NEWS, OperationalAction.GENERATE_DRAFT]) {
...
export const executeVercelLiveProcessingCron = async ({
  ...
  const articleFetcher = createLiveArticleFetcher({ allowedHosts: entries.map((source) => source.domain) });
  const groundedClient = createGeminiGroundedResearchClient(environment);
  ...
  const results: Array<{ candidateId: string; outcome: LiveProcessingOutcome; storyId: string | null }> = [];
  ...
  for (const candidate of pending) {
    ...
    results.push({ candidateId: candidate.id, outcome: result.outcome, storyId });
  }
