...
test('queue item preserves opaque revision for optimistic concurrency', () => {
  const item = buildEditorialQueueItem(EditorialRole.REVIEWER, source('revision'));
  assert.equal(item.revision, 'rev-revision');
});
...
test('queue item carries assessment for every editorial action', () => {
...
test('queue sorts attention buckets first, then score, then update time', () => {
  const items = buildEditorialQueue(EditorialRole.OWNER, [
    source('published', { snapshot: snapshot({ storyStatus: CanonicalStoryStatus.PUBLISHED, publicationStatus: PublicationStatus.PUBLISHED }) }),
    source('review-low', { orbiScore: 80 }),
    source('blocked', { snapshot: snapshot({ storyStatus: CanonicalStoryStatus.BLOCKED, editorialGate: { decision: IntegratedEditorialDecision.BLOCK, editorialBand: OrbiEditorialBand.HOLD, reasons: ['BLOCK'] } }) }),
    source('review-high', { orbiScore: 95 }),
  ]);
  assert.deepEqual(items.map((item) => item.storyId), ['blocked', 'review-high', 'review-low', 'published']);
});

test('queue filter selects bucket without mutating source queue', () => {
  const items = buildEditorialQueue(EditorialRole.OWNER, [
    source('review'),
    source('approved', { snapshot: snapshot({ storyStatus: CanonicalStoryStatus.APPROVED, editorialGate: { decision: IntegratedEditorialDecision.ALLOW_EDITORIAL, editorialBand: OrbiEditorialBand.PRIORITY, reasons: ['ALLOW'] } }) }),
  ]);
